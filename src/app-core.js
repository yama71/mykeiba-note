...
function parseSpecialRegistrationEntries(text) {
  ...
  const netkeibaRows = parseNetkeibaSpecialRegistrationEntries(lines);
  if (netkeibaRows.length > 0) return netkeibaRows;
  const rows = [];
  lines.forEach((line) => {
...
function parseNetkeibaSpecialRegistrationEntries(lines) {
  ...
  for (let index = 0; index < safeLines.length; index += 1) {
    ...
    const popularityLine = safeLines[index + 3] || "";
    const odds = /^\d+(?:\.\d+)?$/.test(oddsLine) ? oddsLine : "";
    const popularity = popularityLine.match(/(\d{1,2})\s*人気/)?.[1] || "";
    rows.push({
      id: crypto.randomUUID(),
      frameNumber: "",
      horseNumber: "",
      horseName: cleanHorseName(horseNameLine || detail.horseName || ""),
...
function normalizeSpecialJockey(value) {
  const jockey = String(value || "").replace(/\s+/g, "").trim();
  if (!jockey || jockey === "未定" || jockey === "〇〇" || jockey === "○○") return "未定";
  return jockey;
}
...
function parseRaceResultMeta(text, raceInfo = {}) {
  ...
  const half = calculateHalfTimes(lapTimes, raceInfo);
  const threeF = calculateThreeFTimes(lapTimes, raceInfo, last3F);
  const firstFurlong = calculateFirstFurlongBase(lapTimes, raceInfo);
  return {
    lapTimes,
    last4F,
    last3F,
...
function calculateHalfTimes(lapTimes, raceInfoOrDistance) {
  const lapData = lapDataForRace(lapTimes, raceInfoOrDistance);
  const segments = lapData.laps.map((lap, index) => ({ time: lap, distance: lapData.distances[index] }));
  if (segments.length === 0) return { firstHalfTime: "", secondHalfTime: "", halfDiff: "" };
  const totalDistance = lapData.totalDistance;
...
function raceInfoValue(source = {}, keys = []) {
...
function numericDistanceFromRaceInfo(source = {}) {
...
function shouldForceFukushima1150DisplayFromLaps(resultLike = {}, raceInfo = {}) {
  const raw = rawRaceResultSource(resultLike);
  const merged = { ...raw, ...raceInfo };
  if (shouldUseFukushimaDirt1150LapLogic(raw, raceInfo)) return true;

  const raceIdText = safeString(raceInfoValue(merged, ["raceId", "id"]), "");
  const racecourse = safeString(raceInfoValue(merged, ["racecourse", "track", "course", "place", "venue"]) || extractRacecourseFromText(raceIdText), "");
  const surface = safeString(raceInfoValue(merged, ["surface", "courseType"]), "");
  const explicitNonFukushima = racecourse && !racecourse.includes("\u798f\u5cf6") && !racecourse.includes("遖丞ｳｶ");
  const explicitNonDirt = surface && !surface.includes("\u30c0") && !surface.includes("繝");
  if (explicitNonFukushima || explicitNonDirt) return false;

  const laps = lapTimesFromResultLike(raw);
  if (laps.length !== 6) return false;

  const distance = numericDistanceFromRaceInfo(merged);
  const storedFirst3F = Number(safeString(raw.first3F || raw.front3F || raw.firstThreeF || raw.lapStats?.front3F || raw.lapStats?.first3F || "").replace(/[^\d.]/g, ""));
  const storedFirstFurlong = Number(safeString(raw.firstFurlongBase || raw.ten1F || raw.tempo1F || raw.lapStats?.ten1F || raw.lapStats?.firstFurlong || safeArray(raw.fullResults || raw.rows || raw.results).find((row) => row?.firstFurlongEstimate && row.firstFurlongEstimate !== "-")?.firstFurlongEstimate || "").replace(/[^\d.]/g, ""));
  const oldFirst3F = laps.slice(0, 3).reduce((sum, value) => sum + value, 0);
  const oldFirstFurlong = laps[0];
  const hasBrokenStoredSplit = roughlyEquals(storedFirst3F, oldFirst3F) || roughlyEquals(storedFirstFurlong, oldFirstFurlong);
  return distance === 1150 || hasBrokenStoredSplit;
}
...
function lapDataForRace(lapTimes, raceInfoOrDistance) {
  const laps = safeArray(lapTimes).map((value) => Number(value)).filter((value) => Number.isFinite(value));
  const distances = lapDistancesForRace(laps, raceInfoOrDistance);
  const numericDistance = numericDistanceFromRaceInfo(raceInfoOrDistance);
  const coveredDistance = distances.reduce((sum, value) => sum + value, 0);
  const totalDistance = numericDistance > 0 ? numericDistance : coveredDistance;
  const shouldEstimateFinalGap = isFukushimaDirt1150Race(raceInfoOrDistance)
    && totalDistance > coveredDistance
    && totalDistance - coveredDistance <= 100
    && laps.length > 0
    && distances.length > 0;

  if (!shouldEstimateFinalGap) {
    return { laps, distances, totalDistance, coveredDistance };
  }

  const gap = totalDistance - coveredDistance;
  const lastDistance = distances[distances.length - 1] || 200;
  const lastLap = laps[laps.length - 1];
  return {
    laps: [...laps, lastLap * (gap / lastDistance)],
    distances: [...distances, gap],
    totalDistance,
...
function calculateThreeFTimes(lapTimes, raceInfoOrDistance, officialLast3F = "") {
  ...
  const numericDistance = numericDistanceFromRaceInfo(raceInfoOrDistance);
  const distances = lapData.distances;
  const totalDistance = lapData.totalDistance;
...
function sanitizeRaceEntry(entry = {}) {
  ...
  return {
    ...
    odds: String(entry.odds || entry.expectedOdds || ""),
...
function sanitizeRaceCard(race = {}) {
  ...
  if (result) result = recalculateFukushimaDirt1150Result(result, raceInfo);
...
function normalizeRaceResultDetails(candidate = {}) {
  ...
  const first3F = safeString(raw.first3F || raw.front3F || raw.firstThreeF || raw.lapStats?.first3F || raw.lapStats?.front3F || "");
  const last3F = safeString(raw.last3F || raw.back3F || raw.final3F || raw.last3f || raw.lapStats?.last3F || raw.lapStats?.back3F || "");
  const threeFDiff = safeString(raw.threeFDiff || raw.paceDiff || raw.threeFDifference || raw.lapStats?.threeFDiff || raw.lapStats?.paceDiff || "");
  return {
    ...raw,
...
function recalculateFukushimaDirt1150Result(resultLike = {}, raceInfo = {}) {
  ...
  if (!shouldForceFukushima1150DisplayFromLaps(resultLike, raceInfo) || result.lapTimes.length === 0) return result;
  ...
  const firstFurlongTopHorseNumbers = safeArray(result.firstFurlongTopHorseNumbers).length > 0
    ...
    : extractHorseNumbersInOrder(result.cornerPassages?.[firstFurlongCorner] || "").slice(0, 5);
  const firstFurlongMap = buildFirstFurlongMap({
    firstFurlongBase,
  ...
  return {
    ...
    fullResults: rows,
    top3: sortedRows.slice(0, 3),
    first3F: threeF.first3F || result.first3F,
    front3F: threeF.first3F || result.front3F,
    last3F: threeF.last3F || result.last3F,
    ...
    isFirst100mStart: true,
    firstFurlongBase: firstFurlongBase || result.firstFurlongBase,
    firstFurlongCorner,
    firstFurlongTopHorseNumbers,
  };
}

function fukushimaLapResultChanged(before = {}, after = {}) {
  const oldResult = normalizeRaceResultDetails(before);
  const newResult = normalizeRaceResultDetails(after);
  const oldRows = safeArray(oldResult.fullResults);
  const newRows = safeArray(newResult.fullResults);
  const oldFirstFurlongs = oldRows.map((row) => `${row.horseNumber}:${row.firstFurlongEstimate || "-"}`).join("|");
  const newFirstFurlongs = newRows.map((row) => `${row.horseNumber}:${row.firstFurlongEstimate || "-"}`).join("|");
  return oldResult.first3F !== newResult.first3F
    || oldResult.last3F !== newResult.last3F
    || oldResult.threeFDiff !== newResult.threeFDiff
    || oldResult.firstFurlongBase !== newResult.firstFurlongBase
    || oldFirstFurlongs !== newFirstFurlongs;
}
...
function resultRowsFromHorseRecordsForRace(race = {}, horseRecords = []) {
  ...
  const linkedResult = shouldForceFukushima1150DisplayFromLaps(resultSource, safeRace)
...
function firstFurlongForHorseRecordDisplay(record = {}) {
  ...
  try {
    ...
    if (!resultSource || !shouldForceFukushima1150DisplayFromLaps(resultSource, record)) return stored;
...
function findResultForRace(race = {}, horseRecords = []) {
  ...
  if (storedResult) return recalculateFukushimaDirt1150Result(storedResult, safeRace);
  if (safeRace.result) return recalculateFukushimaDirt1150Result(safeRace.result, safeRace);
...
function recalculateFukushimaDirt1150Collections(raceCards = [], legacyResults = [], horseRecords = []) {
  ...
  const safeRaceCards = sanitizeRaceCards(raceCards).map((race) => {
    if (!race.result || !shouldForceFukushima1150DisplayFromLaps(race.result, race)) return race;
  ...
  const nextLegacyResults = safeArray(legacyResults).map((item) => {
    ...
    if (!shouldForceFukushima1150DisplayFromLaps(rawResult, race)) return item;
  ...
  const nextHorseRecords = safeArray(horseRecords).map((record) => {
    ...
    if (!shouldForceFukushima1150DisplayFromLaps(result || {}, race)) return record;
...
function resultRowsFromRace(race, horseRecords = []) {
  ...
  const storedMeta = storedResult ? recalculateFukushimaDirt1150Result(storedResult?.result || storedResult, safeRace) : null;
  const storedRows = safeArray(storedMeta?.fullResults || storedMeta?.rows);
  if (storedRows.length > 0) return enrichRowsWithFirstFurlong(storedRows, storedMeta);
...
export function createKeibaApp(React, icons) {
  ...
  function EntryEditor({ entry, index, updateEntry }) {
    return h("article", { className: `entry-card ${entry.parsed ? "" : "unparsed"}` },
      ...
      h("div", { className: "entry-grid" },
        ...
        h(Field, { label: "想定オッズ" }, h("input", { inputMode: "decimal", value: entry.odds || "", onChange: (event) => updateEntry(entry.id, "odds", event.target.value) })),
  ...
  function RaceLapPanel({ result, raceInfo = {} }) {
    const isFukushima1150 = shouldForceFukushima1150DisplayFromLaps(result || {}, raceInfo);
    ...
    if (isFukushima1150 && laps.length > 0) {
      ...
      const rawFirst3F = calculateSegmentTime(
        lapDataForRace(laps, calcInfo).laps,
        lapDataForRace(laps, calcInfo).distances,
        0,
        600
      );
      const rawTen1F = calculateTimeRange(
        lapDataForRace(laps, calcInfo).laps,
        lapDataForRace(laps, calcInfo).distances,
        0,
        200
      );
      console.info(`Fukushima1150 display recalculated: laps=[${laps.join(",")}], front3F=${Number.isFinite(rawFirst3F) ? rawFirst3F : "-"}, ten1F=${Number.isFinite(rawTen1F) ? rawTen1F : "-"}`);
